...
public class AdminController {
	...
	public String loginAction(ModelMap map, HttpServletRequest request,
		...
		long admin = adminService.authenticate(adminId, adminPwd);
		if (admin == ErrorCodes.NONNUMERIC_ID.value) {
			map.addAttribute("error", "ID must be numeric");
			return "admin/login";
		}else if (admin == ErrorCodes.INVALID_ID.value) {
			map.addAttribute("error", "Invalid ID");
			return "admin/login";
		}else if (admin == ErrorCodes.INVALID_PASSWORD.value) {
	...
	public String updatePassword(ModelMap map, @RequestParam(value = "pwd", required = true) String pwd,
			@RequestParam(value = "pwd2", required = true) String pwd2, HttpServletRequest request) {
		// check if session is still alive
		HttpSession session = request.getSession();
		if (session.getAttribute("admin_id") == null) {
			return "admin/login";
